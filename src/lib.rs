...
type Samples = [[f32; NELLY_SAMPLES]; 2];

pub struct NellymoserDecoder<R: Read> {
    pub reader: R,
    pub state: [f32; NELLY_BUF_LEN],
...
impl<R: Read> NellymoserDecoder<R> {
    ...
    pub fn next_frame(&mut self) -> Option<Frame> {
        ...
        let samples = decode_block(&mut self.state, &block, &mut self.planner);

        Some(Frame {
            header: [0f32; NELLY_HEADER_BITS],
            samples,
        })
    }
}
...
pub struct Frame {
    pub header: [f32; NELLY_HEADER_BITS],
    pub samples: Samples,
}
